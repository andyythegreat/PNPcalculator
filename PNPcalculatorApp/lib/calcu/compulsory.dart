...
class CompulsoryScreen extends StatefulWidget {
  ...
  @override
  State<CompulsoryScreen> createState() => _AssistScreenState();
}

class _AssistScreenState extends State<CompulsoryScreen> {
  ...
  String? calculateYearsMonthsDaysExcludingWeekends() {
    if (
    selectedYearOfBirth != null &&
      ...
      DateTime serviceEntryDate = DateTime(
        int.parse(selectedEnteredServiceYear!),
        _monthStringToInt(selectedEnteredServiceMonth!),
        int.parse(selectedEnteredServiceDay!),
      );

      DateTime birthday56 = DateTime(birthday.year + 56, birthday.month, birthday.day);

      int totalWeekdays = 0;
      for (DateTime date = serviceEntryDate; date.isBefore(birthday56); date = date.add(Duration(days: 1))) {
        if (date.weekday != DateTime.saturday && date.weekday != DateTime.sunday) {
          totalWeekdays++;
        }
      }

      int years = (totalWeekdays / 365).floor();
      int months = ((totalWeekdays % 365) / 30).floor();
      int days = (totalWeekdays % 365) % 30;
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          onPressed: () {
                            ...
                            String? result = calculateYearsMonthsDaysExcludingWeekends();
                            if (result != null) {
                              Navigator.push(
                                context,
                                MaterialPageRoute(
